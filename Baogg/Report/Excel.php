 ...
 * @copyright  https://github.com/vernes/PHPReport
 */


class Baogg_Report_Excel {
    ...
    public function generateReport()
    {
		...
		//echo __FILE__.__LINE__.'<pre>';print_r($this);exit;
        foreach($this->_data as $data)
		{
			if(isset ($data['repeat']) && $data['repeat']==true)
			{
				...
				foreach ($this->objWorksheet->getRowIterator() as $row)
				{
					...
					foreach ($cellIterator as $cell)
					{
						...
						if(preg_match_all("/\{".$data['id'].":([\w\(\)（）\/／]*|#\+?-?(\d*)?)\}/u", $cellval, $matches))
						{
    ...
    private function generateSingleRow(& $data)
    {
		...
		{
			$search="{".$id.":".$key."}";
    		if($value instanceOf  PHPExcel_Worksheet_Drawing){
    			$this->_image[$search] = $value;
    			$value = '';
    		}
			$this->_search[]=$search;
	...
	private function generateRepeatingRows(& $data, $repeatRange)
    {
		...
		//echo __FILE__.__LINE__.'<pre>';print_r($repeatRange);print_r($repeatTemplateArray);exit;
	...
	private function generateSingleRepeatingRow(& $value, & $repeatTemplateArray, $rowCounter, $skip, $id, $format, $step)
    {
        foreach($repeatTemplateArray as $rowKey=>$rowData)
        {
            foreach($rowData as $col=>$tag)
            {
                ...
                if(preg_match_all("/\{".$id.":([\w\(\)（）\/／]*|#\+?-?(\d*)?)\}/u", $tag, $matches))
                {
                    ...
                    foreach($matchKeys as $mkey)
                    {
                        ...
                        $replaceTag = "{".$id.":".$mkey."}";
                        if($rValue instanceof PHPExcel_Worksheet_Drawing){
                            $this->_image[$replaceTag] = $rValue;
                            continue;
                        }
                ...
                foreach((array)$this->_image as $replace=>$objDrawing){
                    $objDrawing->setWorksheet( $this->objWorksheet);
                    $objDrawing->setCoordinates($newCellAddress);
	...
	private function searchAndReplace()
    {
        foreach ($this->objWorksheet->getRowIterator() as $row)
		{
			...
			foreach ($cellIterator as $cell)
			{
    			...
    			foreach($this->_image as $search =>$objDrawing){
    				if(strpos($cellValue, $search)!== false){	

    					//echo __FILE__.__LINE__.'<Pre>';var_dump($cell->getCoordinate());exit;
    					$pos = $cell->getCoordinate();
    					$objDrawing->setWorksheet($this->objWorksheet);
    			...
    			$cell->setValue(str_replace($this->_search, $this->_replace,$cellValue ));
    ...
    {
    	//echo __FILE__.__LINE__.'<pre>';var_dump($this->_usingTemplate);exit;
